...
def build_system_prompt(
    ...
    if action_mode in ("confirm", "auto"):
        lines.append("=== 处置建议格式 ===")
        lines.append("每行格式：操作类型 #N 秒数 原因 | 通知内容")
        lines.append("操作类型: 禁言、移除、拉黑、清昵、撤回")
        lines.append("禁言秒数自行判断，建议范围 300-7200s")
        lines.append("| 后直接写要发送的消息，不要加\"群内通知:\"等前缀")
        lines.append("")
        lines.append("合并规则：")
        lines.append("- 同一成员同一操作类型只能出现一次，直接将最终秒数填入")
        lines.append("- 不要写公式、计算过程、括号内备注")
        lines.append("- 反例：禁言 #14 秒(刷屏)+600秒(挑衅)=1200秒（600s）| 群内通知: ...")
        lines.append("- 正例：禁言 #14 1200 多次刷屏和挑衅 | 请遵守群规文明交流")
        lines.append("涉及管理员的处置会被跳过")
        lines.append("")
