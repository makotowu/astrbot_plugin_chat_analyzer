...
def build_combined_prompt(
    ...
    lines.append("  4. 提醒内容要求：")
    lines.append("     - 管理员相关的提醒话术只能写在【管理员提醒】章节，不能混写到【定位清单】【处理建议】【处置建议】里。")
    lines.append("     - 只要 [管理员] 成员存在需要关注的异常发言，就必须补一条对应的【管理员提醒】；不要漏写。")
    lines.append("     - 说话像同事之间私下提个醒，友善、自然、有分寸，不要像下命令，也不要像公开批评。")
    lines.append("     - 根本目的：约束不当服务行为，提醒管理员注意服务态度、沟通方式和边界意识。")
    lines.append("     - 语气温和但坚定，可多用建议、可以、咱们、这边这类软性措辞，让人愿意接受。")
    lines.append("     - 聚焦当次行为本身，提醒改进即可，不上纲上线，不进行人格评价，不夸大后果。")
    lines.append("     - 不要写任何平台动作，不要写 @、通知群、处置、处罚、撤回、禁言、移出等执行性词语。")
    lines.append("     - 文案尽量像真人会说的话，简短一点，避免模板腔、公告腔、执法通报腔。")
    lines.append("     - 示例：提醒 #2 这边建议稍微注意一下回应方式，避免让群成员感觉被怼，咱们一起把氛围稳住。")
    lines.append("")
    if group_rules:
        lines.append("【群规参考】")
    ...
    lines.append("9. 涉及 [管理员] 成员时，【定位清单】只负责定位编号和抽象原因，具体提醒话术必须单独写进【管理员提醒】。")
    lines.append("10. 【定位清单】【处理建议】【处置建议】都不要出现对管理员说的话、第二人称提醒句或任何类似私信文案。")
    lines.append("")
    lines.append("【总体结论】")
    ...
    lines.append("这里只写提醒文案本身，不写 @，不写通知群，不写处罚动作，不写执行说明。")
    ...
    if action_mode in ("confirm", "auto"):
        ...
        lines.append("")
        lines.append("重要：同一成员多条处置的群内通知合并规则：")
        lines.append("  1. 同一成员如果有多条处置建议，群内通知只写在第一条处置的竖线后面。")
        lines.append("  2. 第一条的群内通知要概括该成员的所有处置，用一句自然的话说完（撤回几条消息、禁言多久等）。")
        lines.append("  3. 该成员后续处置行的竖线后只写\"同上\"，不要重复写通知。")
        lines.append("  4. 示例：")
        lines.append("     撤回 #7 广告导流需撤回 | 涉及广告的消息已撤回，并禁言半小时，请注意群规～")
        lines.append("     撤回 #8 重复广告导流需撤回 | 同上")
        lines.append("     禁言 #7 1800 发布广告引导 | 同上")
        lines.append("")
        lines.append("处置建议示例：")
