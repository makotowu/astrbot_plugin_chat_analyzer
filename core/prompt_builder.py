...
def build_combined_prompt(
    ...
    if action_mode in ("confirm", "auto"):
        ...
        lines.append("  1. 操作类型严格限定为 禁言、移除、拉黑、清昵 四个词，不得使用提醒、警告、at、全体、群公告等任何其他词。")
        lines.append("  2. 每行一条，格式为：{操作类型} #N [秒数] 简短原因 | 群内通知")
        lines.append("  3. 只列出需要对\"具体成员\"执行 QQ 平台操作（禁言/移除/拉黑/清昵）的项目。")
        lines.append("  4. 如果某个问题只需全员文字提醒、不需要对任何人执行禁言/移除/拉黑/清昵，请写入【处理建议】而非【处置建议】。")
